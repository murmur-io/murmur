...
|---|---|---|---|
| L1 | ~~Medium~~ → **Low (ZMIERZONE)** | **Sidecar mistralrs**: cały wątek silnika owinięty jednym `autoreleasepool`, drenowanym dopiero przy wyjściu wątku (`mistralrs-core-0.8.1/src/lib.rs:570-573`); jedynym „drenem" jest idle-kill po 300 s. Seria Ask w odstępach < 5 min nigdy go nie odpala. | `while :; do ps -o rss= -p $(pgrep -x meetnotes-brain); sleep 5; done` przez 1 → 10 → 30 zapytań; monotoniczny wzrost resetujący się po 5 min bezczynności = to. **ZMIERZONE 2026-09-04** (`scripts/measure-sidecar-rss.py`, Qwen3-1.7B-Q4_K_M, build release aarch64): hipoteza potwierdzona CO DO KSZTALTU, obalona CO DO WAGI. Kazda z 30 probek >= poprzedniej — monotonicznie, dokladnie jak przewiduje niedrenowany `autoreleasepool`. Ale dryf to **0,66 MB na zapytanie** (4186 MB po 1. generacji, 4207 MB po 30.), czyli kilkadziesiat MB w normalnej sesji. Rezydentny model to PODLOGA **~4,2 GB** — to ona jest kosztem, nie dryf ponad nia. Proponowany backstop „respawn co N zadan" kupilby dziesiatki MB za 2,7 s przeladowania modelu: **nieoplacalny**. Jesli wracac do pamieci sidecara, celem jest podloga (mniejszy model, unload przy bezczynnosci), nie wyciek. |

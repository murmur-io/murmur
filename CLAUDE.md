...
6. Merge to `murmur` **via a PR, never a direct push** (enforced by `.claude/hooks/block-bash.sh`); commits/PRs are authored by **the person who actually opens the PR** — their own git identity and their own `gh` account (confirm `git config user.name`/`user.email` and `gh auth status` before committing; never borrow another maintainer's), **no Claude trailers**; `com.meetnotes.app` immutable.

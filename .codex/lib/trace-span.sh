#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# trace-span.sh — append one JSONL span to .codex/tmp/<task>/trace.jsonl.
# Best-effort observability for a ship-feature run. NEVER blocks, always exit 0.
#
# Usage: .codex/lib/trace-span.sh <task> <phase> <gate> <verdict> [artifact] [durationMs]
#   e.g. .codex/lib/trace-span.sh murmur-hooks verify adversarial-verify PASS adversarial-verify.json 41200
#
...
[ -z "$task" ] && exit 0
dir=".codex/tmp/$task"; mkdir -p "$dir" 2>/dev/null || exit 0

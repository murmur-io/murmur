...
#
# Murmur's DoD is binding (AGENTS.md): the verifier owns the verdict, not the
...
#
# Gate files live in .codex/tmp/<task>/  (written by the adversarial-verifier and
# lock-security-reviewer per .codex/agents/*.toml):
...
if [ "$mode" = "enforce" ]; then
  if [ "$is_codex_payload" = "1" ]; then
    jq -cn --arg reason "finish-guard: Definition-of-Done not met for task '$task':$missing" '{decision:"block",reason:$reason}'
    exit 0
  fi

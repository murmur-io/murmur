...
#   - .codex/hooks/: the guardrail scripts + selftest fixtures necessarily contain
...
# (MURMUR_DEV_DEK/KEK lines + the fixed 0123…ef dev key that lives in AGENTS.md by design).
hex64="$(printf '%s' "$diff" \
...
if [ -n "$hits" ]; then
  if [ "$is_codex_payload" = "1" ]; then
    jq -cn --arg reason "secret-scan blocked the commit; staged diff appears to contain secret material: $hits" '{decision:"block",reason:$reason}'
    exit 0
  fi
  {

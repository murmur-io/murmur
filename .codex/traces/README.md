# `.codex/traces/` — per-task evidence archive

In-flight run artifacts (gate JSONs, `trace.jsonl` spans) live in `.codex/tmp/<task>/`, which is
gitignored scratch. When a task lands and you want a durable, reviewable record, archive that dir
here as `.codex/traces/<task>/` with a one-line `MANIFEST.txt`:

```
task=<slug> outcome=PASS|FAIL branch=<branch> archived_at=<UTC>
```

This is optional and manual today — the machine gate (`finish-guard.sh`) reads from `.codex/tmp/`,

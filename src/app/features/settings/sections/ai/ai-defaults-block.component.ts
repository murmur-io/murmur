...
@Component({
  ...
  template: `
    <div class="card defaults-card" [formGroup]="form">
      ...
      <!--
        Installed-base migration nudge (deliverable #3): the persisted on-device
        model was RETIRED for licensing → offer the Apache-licensed replacement.
        In-flow alert, so the frosted .banner is correct (no opaque overlay).
      -->
      ...
      <!--
        Murmur Brain posture (deliverable #2): a 3-way preset selector reading the
        DERIVED brain_posture() into a signal. "custom" is a display-only state
        (no preset forced) — its own pill shows and no segment is active.
      -->
      <div class="posture">
        ...
        <span class="field-help text-muted">
          Your Default AI still writes all notes; realtime reactions run locally —
          like transcription, this lives on your Mac.
        </span>
        ...
        <!--
          Brain Live enablement — only when the posture is Cloud (no on-device
          reactions yet). One tap switches to Hybrid AND downloads/selects the
          smallest LIGHT model. Copy is CONDITIONAL on cloud consent (spec §2.2).
        -->
        @if (posture() === "cloud") {
          <div class="brain-live-card">
            <div class="brain-live-copy">
              <span class="brain-live-title">Enable Murmur Brain Live</span>
              <span class="text-secondary brain-live-sub">
                Realtime reactions run locally — like transcription, this lives on
                your Mac.
                @if (cloudConsented()) {
                  Fact extraction moves fully local (today it rides your cloud AI,
                  redacted).
                } @else {
                  Fact extraction turns on, fully local.
                }
              </span>
              @if (!brainLiveModelReady()) {
              ...
              @if (brainDownloadingId()) {
                <div class="semantic-progress" role="status">
                  ...
                  <span class="semantic-progress-label text-muted">
                    Downloading Brain Live model… {{ brainPct() }}
                  </span>
                </div>
              } @else {
  ...
  styles: [
    `
      ...
      /* Brain Live enablement — copy + action, stacked. */
      .brain-live-card {
        display: flex;
        flex-direction: column;
        gap: var(--space-3);
...
export class AiDefaultsBlockComponent {
  ...
  // Murmur Brain — posture chooser + Brain Live enablement + retirement nudge.
  ...
  /** One-tap "Enable Brain Live": Hybrid preset + download/select the light model. */
  enableBrainLive(): void {
    void this.store.enableBrainLive();
  }
  ...
  /** "~N GB" label for the Brain Live one-time download (from the smallest light model). */
  brainLiveSizeLabel(): string {
    const bytes = this.store.brainLiveModelBytes();
    if (bytes == null) return "1.1 GB";

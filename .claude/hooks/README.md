...
| `autoformat.sh` | PostToolUse(Edit\|Write) | `rustfmt` the edited `.rs` file. FE formatting stays with `npx ng lint` (the zoneless rules — inline templates, per-component style budget — a blind prettier pass would fight them). | **off** (`MURMUR_AUTOFMT=1` to enable) |

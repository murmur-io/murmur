# `.claude/hooks/` — deterministic guardrail adapters

Claude and Codex call the same parser and verifier in
`.agents/harness/hook_guard.py`. The shell files here only translate the Claude
hook payload and exit contract; policy must not be reimplemented in an adapter.
...
The selftest runs the same canonical implementation against both Claude and Codex

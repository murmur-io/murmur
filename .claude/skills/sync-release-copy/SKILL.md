...
1. **`gh` active account MUST be your own** (`gh auth status`). If several accounts are
   logged in, switch to yours (`gh auth switch`).
2. **Commit/PR author MUST be the person actually opening the PR** (your own git identity).
   **NO Claude trailers**, no `Co-Authored-By: Claude`, anywhere.
...
6. **Truthfulness bar** — a claim the current code does NOT do is a bug, not a nicety.
   ...
   `.claude/rules/lock-model.md`): "nothing leaves the device", "two encryption layers",

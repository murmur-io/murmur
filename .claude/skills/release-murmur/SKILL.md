...
2. **Commit author MUST be the person cutting the release** — your own `git config user.name` /
   `user.email`, not whoever committed last. NO `Co-Authored-By: Claude`,
   NO Claude trailers anywhere in release commits/PR bodies.
...
## Stage 3 — Commit as yourself (no Claude trailers)

```bash
git add package.json package-lock.json src-tauri/tauri.conf.json src-tauri/Cargo.toml Cargo.lock
git commit -m "chore(release): bump version to $NEW"
git log -1 --format='%an <%ae>%n%b'   # author = you (the releaser), body has NO Co-Authored-By / Claude
```
...
- **Identity is non-negotiable:** gh=the releaser's own account, commit-author=the releaser (no Claude trailers),

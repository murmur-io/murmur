...
- **Return the structured verdict to the harness; write no verdict files.** `.agents/h/h.py` invokes every reviewer in a fresh read-only process through the reviewer-only runtime, validates its schema, hashes the artifact, and creates the only authoritative evidence receipt under the shared Git common directory. Scratch `.claude/tmp/` JSON cannot clear the commit guard.

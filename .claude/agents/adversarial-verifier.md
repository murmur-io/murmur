...
- **Return the structured verdict to the harness; write no verdict files.** `.agents/harness/task_runner.py` invokes every reviewer in a fresh read-only process, validates its schema, hashes the artifact, and creates the only authoritative attestation under the shared Git common directory. Legacy `.claude/tmp/` JSON cannot clear the commit guard.

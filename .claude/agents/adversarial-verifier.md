...
- **Emit the machine-readable verdict** (in addition to the prose report above). When dispatched with a task scratch dir, write via Bash into the gitignored scratch `.claude/tmp/<task>/adversarial-verify.json` = `{"verdict":"PASS"|"FAIL","findings":[{"severity":"…","file":"…","evidence":"…"}],"summary":"…"}`. This JSON is what `.claude/hooks/finish-guard.sh` gates the commit on — the DoD becomes a machine check, not the committer's eyeball. (Optional span: `.claude/lib/trace-span.sh <task> verify adversarial-verify <verdict> adversarial-verify.json`.) This is scratch, not app code — the read-only rule is intact.

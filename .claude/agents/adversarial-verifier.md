...
2. **Run the real static gates** (do not trust the author's claim they pass):
   - `scripts/agent-resource-run --chdir src-tauri -- cargo test --lib` — the canonical Rust loop. Never run Cargo/rustc outside the wrapper.
   - `npx ng lint` and `scripts/agent-resource-run -- npx ng build` — for any FE change. A failing build/lint is an immediate FAIL.
...
- **Return the structured verdict to the harness; write no verdict files.** `.agents/harness/verifier.py` invokes every reviewer in a fresh read-only process through the reviewer-only runtime, validates its schema, hashes the artifact, and creates the only authoritative evidence receipt under the shared Git common directory. Scratch `.claude/tmp/` JSON cannot clear the commit guard.
